...
export function isFreeParking(loc: ParkingLocation): boolean {
  // Street parking in our dataset has no per-hour pricing tiers, so we
  // explicitly treat it as "free" only after 5:30pm.
  if (loc.type === 'street') {
    const now = new Date();
    const mins = now.getHours() * 60 + now.getMinutes();
    return mins >= FREE_AFTER_MINUTES;
  }
...
export const initialParkingData: ParkingLocation[] = [
  ...
  },
  {
    ...
    lat: 44.23061,
    lng: -76.48019,
    address: 'Ontario St W of Princess',
    operatingHours: [{ days: ALL_DAYS, start: '00:00', end: '23:59' }],
    pricing: {
      tiers: [
        {
  ...
  },
  {
    ...
    lat: 44.228758,
    lng: -76.491901,
    address: '45 Union St W, Kingston, ON K7L 3N6',
    evCharging: 'level2',
    accessibleParking: true,
    operatingHours: [{ days: ALL_DAYS, start: '00:00', end: '23:59' }],
